...
class BillManager extends CI_Model {
    ...
    public function getAppBillList($strAccountId, $pn, $rn) {
        ...
        $strSqlSelectMediaName = 'SELECT app_id,media_name FROM media_info WHERE account_id=\'' . $strAccountId . "'";
        ...
        foreach($arrMediaNames as $arrMediaName) {
            $arrFormatMediaNames[$arrMediaName['app_id']] = $arrMediaName['media_name'];
        }

        $arrRes = [];
        foreach ($arrAppMoney as $arrMoney) {
            $arrRes[] = [
                'media_name' => $arrFormatMediaNames[$arrMoney['app_id']],
